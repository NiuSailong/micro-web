...
import { findRegionPage, addRegion, updateRegion, deleteRegion } from '@/services/area';
...
const TableList = (props) => {
  ...
  let add = (record) => {
    ...
    // arr = { abbreviation: '请输入', provinceName: '请输入', provinceId: '', key: `add${data.length}` }
    arr = {
      companyNum: '',
      createId: '--',
      name: '--',
  ...
  let init = async (size, index) => {
    setSpinnIng(true);
    if (props.type == '省') {
      let data = await findRegionPage({ currentPage: index || pageindex, size: size || pagesize });
  ...
  let sumbitdata = async () => {
    ...
    if (addids.length) {
      ...
      for (let i = 0; i < listitem.length; i++) {
        if (addids.indexOf(listitem[i].key) > -1) {
          ...
          delete list.key;
          delete list.createId;
          delete list.name;
          // list.createId = 0;
          list.powerUnitId = parseInt(list.powerUnitId);
          list.regionId = parseInt(list.regionId);
          params.push(list);
        }
      }
      let data = await addRegion(params);
    ...
    if (editids.length) {
      ...
      let res = await updateRegion(listdata);
      if (res.statusCode == '1000') message.success('更新成功');
      if (data.statusCode == '1008') message.error('参数错误添加失败');
    }

    if (dellists.length) {
      let data = await deleteRegion(dellists);
