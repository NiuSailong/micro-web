...
class ModelAlert extends Component {
  ...
  handleOk = () => {
    ...
    if (listTitle.title === '故障编号') {
      const historyCode = duplicate.filter((i) => {
  ...
  handleTableChange = async (pagination, filters, sorter) => {
    ...
    const res = await this.props.serverType(objs);
    if (res.result === 'OK') {
  ...
  seachTable = async (e) => {
    if (e === 'serch') {
      ...
      const res = await this.props.serverType(value);
      ...
      obj.faultName = '';
      obj.faultCode = '';
      const res = await this.props.serverType(obj);
      obj.total = res.total;
      this.setState({
        dataSou: res.dataResults || res.results || [],
        value: obj,
  ...
  selectRow = (selectedRowKeys, selectedRows) => {
    const selectedRowKeyL = [selectedRowKeys[selectedRowKeys.length - 1]];
  ...
  render() {
    ...
    const rowSelection = {
      onChange: (selectedRowKeys, selectedRowss) => {
        this.selectRow(selectedRowKeys, selectedRowss);
      },
    };
    return (
      <Modal
